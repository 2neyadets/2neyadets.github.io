<template lang="pug">
  .mailing-delay__block.filter__block.full-width.q-mx-xs
    .flex.justify-center.items-center.text-center.q-mt-xs
      span Просрочка
    .row.justify-around.items-center
      q-input.col-2.q-mx-xs.max-w-75px(
        v-model.number="$managerWB.mailing.filters.delay.min"
        type="number"
        @change="validateRange($managerWB.mailing.filters.delay)"
      ...
      q-range.w-50.q-pa-none(
        v-model="$managerWB.mailing.filters.delay"
        :min="min"
        :max="max"
        color="positive"
      )
      q-input.col-2.q-mx-xs.max-w-75px(
        v-model.number="$managerWB.mailing.filters.delay.max"
        type="number"
        @change="validateRange($managerWB.mailing.filters.delay)"
        input-class="text-center text-WB-bg-profile-color q-pa-none"
        standout="bg-WB-secondary text-WB-secondary"
...
export default {
  ...
  created () {
    this.$managerWB.mailing.filters.delay.min = this.min
    this.$managerWB.mailing.filters.delay.max = this.max
  },
  mounted () {
  },
  computed: {
